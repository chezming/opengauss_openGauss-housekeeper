...
class ClusterStateChecker():
    ...
    def getClusterListenipConfig(self):
        ...
        for i in range(3):            # 如果失败，尝试3次
            (rst, exeOut) = sshClient.execute(self.sshClients, 0, "GET_CLUSTER_LISTENIP_CONFIG", [config.dbDatanodePaths])
            if(rst): break
            ...
            for idx in range(dbNodesSize):
                if(context.getFloatIpState(idx) == const.STANDBY_UNCLEAR_FLOATIP):  # 说明该节点浮动IP没有消除成功，需获取其sshClient进行
                    ...
                    (rst, rsp) = sshClientTmp.execute(self.sshClients, idx, "CLEAR_NODE_FLOATIP_BUILD", 
                                    [self.dbNodeListenIps[idx], self.dbNodeNames[idx], config.dbDatanodePaths])
                    if (not rst or not Util.parseRefreshClusterConfMsg(rsp)): 
    ...
    def confirmPrimaryFloatIp(self, primaryNodeId):
        ...
        if(not rst):
            system_log.fatal("Can not get ssh connection to primary nodeId %d to confirm float ip, system exit" % (primaryNodeId + 1))
            os._exit(-1)
            
        (rst, msg) = sshClient.execute(self.sshClients, primaryNodeId, 'CONFIRM_FLOATIP_NETWORK', [config.floatipEth, config.floatIp, config.dbNodePort])
        if(not rst):
            system_log.fatal("ssh connect to primary nodeId %d to confirm float ip failed, system exit" % (primaryNodeId + 1))
            os._exit(-1)
        
        if(msg.startswith('1#')):
            system_log.info("successfully confirmed primary node has float ip %s on network card %s" % (config.floatIp, config.floatipEth))
        else:
            system_log.info("Find primary node has no float ip %s on network card %s or not become effective in postgresql.conf" % (config.floatIp, config.floatipEth))            
    ...
    def buildStanbyForNode(self, nodeId):
        ...
        (rst, msg) = sshClient.execute(self.sshClients, nodeId, 'BUILD_AS_STANDBY_NODE', [config.dbDatanodePaths, config.dbNodePort])
    ...
    def recoveryPrimaryNodeBeforeUnaviable(self, oldPrimaryNodeId, currDbClusterState):
        ...
        if(not rst): 
            ...
            system_log.fatal("The database process is existed on nodeId %s, please check its ssh newwork card." % (oldPrimaryNodeId + 1))
            return True
        else:
            system_log.fatal("The nodeId %s state is unknown and can not connect to it, it needs manual support" % (oldPrimaryNodeId + 1))
            return True
    ...
    def recoveryUnknownNode(self, nodeId, targetState):
        ...
        if(not rst): return False 
            ...
            system_log.fatal("There may be have ssh network card failure in the nodeId %s, it needs manual support" % (nodeId + 1))
            return False
    ...
    def getCandidatePrimary(self):
        ...
        if(standbyNodesCount == 0):
            ...
            system_log.info("DB Cluster '%s' has only one standby node before become unavailable, so"
                            "it is the candidate primary node" % str(self.dbClusterBeforeUnaviable))
            ...
            for nodeId in standbyNodeIdsBeforeUnaviable:
                (rst, sshClient) = self.getSSHClient(nodeId)
                if(not rst):
                    system_log.fatal("Cannot query term and lsn from nodeId %d because can not get ssh connect,"
                                     "\nit needs manual support!" % (nodeId + 1))
                    return (False, -1)
        
                (rst, msg) = sshClient.execute(self.sshClients, nodeId, "QUERY_NODE_TERM_LSN", [config.dbNodePort])
                if(not rst):
                    system_log.fatal("Querying term and lsn from nodeId %d failed for the reason:\n%s"
                                     "\nit needs manual support!" % ((nodeId + 1), msg)) 
                elif(msg.startswith('1#')):
                    (rst, termlsn) = Util.parseTermLsn(msg[2:])
                    if(not rst):
                        system_log.fatal("Parase term lsn from '%s' in nodeId %d, it needs manual support!" % (msg[2:], (nodeId + 1)))
                        return (False, -1)
                    ...
                    system_log.fatal("Querying term lsn failed in nodeId %d, the reason:\n%s,\nit needs manual support!" % ((nodeId + 1), msg[2:]))
    ...
    def processStatusUnavailable(self, currDbClusterState):
        ...
        system_log.info("recover the nodeId %s to primary failed, so system will find the candidate primary node and make it to primary." % (primaryNodeIdBeforeUnaviables[0] + 1))
        ''' 找到候选主节点进行主备切换 '''
            ...
            while True:
                ...
                if(currDbClusterState != None):  
                    ...
                    # 在初始进入unavailable状态时，保留之前的集群状态，以便必要时恢复故障前主机
                        ...
                        self.dbClusterBeforeUnaviable = copy.deepcopy(self.lastDbCluster)
